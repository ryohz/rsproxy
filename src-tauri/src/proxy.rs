...
use crate::{
    http_util,
    types::{BodyString, HeadersJson, MethodString, Request, Response, StatusNumber, UrlString},
};
...
//  Result<hyper::Response<hyper::Body>, hyper::Response<hyper::Body>>
{
    let request = if pilot_state(shared_pilot_state.clone()) {
        ...
        rq_front.send_to_front(&app_handle);
        let m_rq_front = rq_front.wait_for_modification(&app_handle).await;
        let rq = m_rq_front.to_hyper().await;
        rq
    } else {
...
fn pilot_state(shared_pilot_state: Arc<Mutex<bool>>) -> bool {
    let sps = shared_pilot_state.lock().unwrap();
    *sps
}
